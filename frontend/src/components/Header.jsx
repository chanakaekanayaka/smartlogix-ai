    ...
    <header className="sticky top-0 z-40 border-b border-white/[0.06] bg-gradient-to-r from-navy-950/90 via-navy-900/88 to-navy-800/85 shadow-[0_1px_0_0_rgba(255,255,255,0.04),0_8px_30px_-12px_rgba(0,0,0,0.5)] backdrop-blur-xl">
      ...
      <div className="mx-auto flex max-w-7xl items-center gap-3.5 px-4 py-3.5 sm:px-6">
        <button
          ...
          className="flex min-w-0 items-center gap-3.5 rounded-lg text-left transition-opacity hover:opacity-80"
        >
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {onNavigate && (
            <button
              type="button"
              onClick={() => onNavigate(view === 'pricing' ? 'app' : 'pricing')}
              className="rounded-full border border-white/10 bg-white/[0.06] px-3.5 py-1.5 text-xs font-semibold text-white transition hover:bg-white/[0.12]"
            >
              {view === 'pricing' ? 'Back to app' : 'Business pricing'}
            </button>
          )}
        </div>
      </div>
